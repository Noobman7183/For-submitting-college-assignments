  ...
  return (
    <View style={styles.container}>
      <Video
        ...
        rate={rate}
      />
      <Text style={styles.timeStamp}>{`${formatTime(currentTime)} / ${formatTime(duration)}`}</Text>
      <Slider
        style={styles.slider}
        value={currentTime}
        maximumValue={duration}
      ...
      />
      <View style={styles.controlRow}>
        <TouchableOpacity style={styles.button} onPress={() => videoRef.current.seek(Math.max(0, currentTime - 10))}>
          <Icon name="stepbackward" size={24} color="white" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={togglePlay}>
          <Icon name={isPlaying ? "pausecircle" : "playcircleo"} size={24} color="white" />
        </TouchableOpacity>
        <TouchableOpacity style={styles.button} onPress={() => videoRef.current.seek(Math.min(duration, currentTime + 10))}>
          <Icon name="stepforward" size={24} color="white" />
        </TouchableOpacity>
      </View>
      <TouchableOpacity style={styles.button} onPress={() => setRepeat(!repeat)}>
        <Icon name="retweet" size={24} color={repeat ? "white" : "gray"} />
      </TouchableOpacity>
      <Slider
        style={styles.slider}
        value={rate}
...
const styles = StyleSheet.create({
  ...
  timeStamp: {
    color: 'white',
    marginVertical: 10, // Add some vertical space above and below the timestamp
  },
});
